...
\begin{itemize}
  ...
  \item Треугольники проверяются на~вырожденность по~формуле площади Гаусса (порог~150).
...
\begin{itemize}
  ...
  \item Оптимизатор Adam ($\mathrm{lr}=10^{-4}$), косинусный отжиг скорости обучения.
  \item Каузальная маска внимания запрещает «заглядывать вперёд».
...
\begin{itemize}
  ...
  \item Алгоритм REINFORCE с~скользящим базлайном (снижение дисперсии).
...
\begin{itemize}
  ...
  \item Постобработка: дедупликация близких по~IoU и~цвету предсказаний.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Результаты}
\framesubtitle{Пример точной реконструкции сцены (9~фигур)}
\begin{figure}[h]
\includegraphics[width=11cm,keepaspectratio]{result-good}
\end{figure}
\centering
...
\framesubtitle{Типичные расхождения модели}
\begin{figure}[h]
\includegraphics[width=11cm,keepaspectratio]{result-errors}
\end{figure}
\centering
\small Малая фигура пропущена (False Negative), близкий дубликат добавлен лишним (False Positive).
\end{frame}
...
\framesubtitle{Готов ответить на~вопросы}
\end{frame}
